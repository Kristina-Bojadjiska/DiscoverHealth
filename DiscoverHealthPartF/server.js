// D:\Project WAD\DiscoverHealth\DiscoverHealthPartF\server.js
...
/* ------------------------------------------------------------------ */
/* GET /api/resources  → return resources WITH reviews array (Part F)  */
/* ------------------------------------------------------------------ */
app.get('/api/resources', (req, res) => {
  ...
  try {
    ...
    // attach reviews for each resource (array of strings)
    const reviewStmt = db.prepare('SELECT review FROM reviews WHERE resource_id = ?');
    const withReviews = list.map(r => ({
      ...r,
      reviews: reviewStmt.all(r.id).map(x => x.review)  // always an array
    }));
...
/* ---------------------------------------------------------------------- */
/* POST /api/resources/:id/reviews  → add a review (auth required) (F12)  */
/* ---------------------------------------------------------------------- */
app.post('/api/resources/:id/reviews', isAuthenticated, (req, res) => {
  ...
  try {
    ...
    const exists = db.prepare('SELECT id FROM healthcare_resources WHERE id = ?').get(id);
    if (!exists) return res.status(404).json({ error: 'Resource not found' });

    const info = db.prepare('INSERT INTO reviews (resource_id, user_id, review) VALUES (?, ?, ?)')
      .run(id, req.session.userId, review.trim());

    res.status(201).json({ id: info.lastInsertRowid, message: 'Review added' });
  } catch (e) {
    console.error('[POST /api/resources/:id/reviews] DB error:', e);
